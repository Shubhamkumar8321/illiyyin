    ...
    <header className="fixed top-0 left-0 w-full z-50 backdrop-blur-[5px] bg-white/60">
      <nav className="flex items-center justify-between px-4 md:px-24 py-4 relative">
        ...
        <div className="flex items-center gap-2 md:gap-4 flex-1 justify-end">
          <div className="hidden md:flex relative items-center border border-[#094C3B] rounded-full overflow-hidden max-w-xs flex-row">
            <input
              type="text"
              value={searchQuery}
...
//       const storedUser = localStorage.getItem("user");
//       if (storedUser) {
//         queueMicrotask(() => setUser(JSON.parse(storedUser) as User));
//       }
//     } catch {
...
//             <button
//               onClick={() => setModalOpen(true)}
//               className="border border-[#094C3B] text-[#094C3B] px-4 py-1 font-bold rounded-full hover:bg-[#094C3B] hover:text-white transition text-"
//             >
//               Login
//             </button>
//           ) : (
...
//                     exit={{ opacity: 0, scale: 0.95, y: -5 }}
//                     transition={{ duration: 0.2 }}
//                     className="absolute mt-3 w-64 bg-white shadow-2xl rounded-2xl border p-4"
//                   >
//                     <div className="flex items-center gap-3 mb-3">
//                       <div className="w-12 h-12 rounded-full bg-[#094C3B] text-white flex items-center justify-center font-bold text-lg">
//                         {user.name?.charAt(0).toUpperCase()}
